...
const mpotContract =  contractFactory.create(mpotAddress,tokenAbi,signer);
...
const main = async function(){   
    ...
    {
      let mpotBalance = await mpotContract.balanceOf(addresses[i]);
      let tcpBalance = await tcpContract.balanceOf(addresses[i]);

      console.log(`Account ${addresses[i]} has balance of ${ethers.utils.formatEther(mpotBalance)} MPOT`);
      console.log(`Account ${addresses[i]} has balance of ${ethers.utils.formatEther(tcpBalance)} TCP`);

      if (mpotBalance.isZero()){

          console.log("Sending MPOT to", addresses[i]);

          const tx = await mpotContract.transfer(
            addresses[i], 
            ethers.utils.parseEther(config.MPOT).mul(config.orbCount), 
            {
          ...
          mpotBalance = await mpotContract.balanceOf(addresses[i]);
          console.log(`Account ${addresses[i]} has balance of ${ethers.utils.formatEther(mpotBalance)} MPOT`);
      }
